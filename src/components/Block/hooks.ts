...
import { computeBlockStyles } from '../../utils/blockStyles'
...
export function useBlock({ block, appearance, scale, onCopy, onChange, onResizeEnd, onWiggleChange }: UseBlockProps) {
  ...
  const { wiggleAmplitude, bgColor, textColor, canvasTextColor, canvasMutedColor } = computeBlockStyles(block, appearance, blockWidth)
